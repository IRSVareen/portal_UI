...
export class AppComponent implements AfterViewInit {
    ...
    visionTemperature: number = 78;
    panelTemperature: number = 85;
    leftCameraTemperature: number = 58;
    rightCameraTemperature: number = 88;
    ...
    updateValue() {
        requestAnimationFrame(() => {
            ...
            this.cpuTemperature = parseFloat((this.cpuTemperature + Math.random() * 2).toFixed(2));
            this.gpuTemperature = parseFloat((this.gpuTemperature + Math.random() * 2).toFixed(2));
            this.visionTemperature = parseFloat((this.visionTemperature + Math.random() * 2).toFixed(2));
            this.panelTemperature = parseFloat((this.panelTemperature + Math.random() * 2).toFixed(2));
            this.leftCameraTemperature = parseFloat((this.leftCameraTemperature + Math.random() * 2).toFixed(2));
            this.rightCameraTemperature = parseFloat((this.rightCameraTemperature + Math.random() * 2).toFixed(2));
            this.conveyorSpeed = parseFloat((this.conveyorSpeed + Math.random() * 2).toFixed(2));
            this.motorAmpere = parseFloat((this.motorAmpere + Math.random()).toFixed(2));
            this.airPressure = parseFloat((this.airPressure + Math.random() * 2).toFixed(2))
        });
    }
    ...
    getConveyorImage() {
      ...
      getVisionImage() {
        if(this.isStop){
            return "/assets/icons/vision.png"
        }

        if (this.visionTemperature > 70 && this.visionTemperature < 80) {
          return "/assets/icons/vision-warning.png";
        } else if (this.visionTemperature > 80) {
          return "/assets/icons/vision-error.png";
        }
        return "/assets/icons/vision-running.png";
      }
      ...
      getMotorImage() {
        ...
        if (this.motorAmpere > 6.00 && this.motorAmpere < 7.12) {
      ...
      getAirPressureImage() {
        ...
        if (this.airPressure > 7.40 && this.airPressure < 8.00) {
          return "/assets/icons/air-warning.png";
        } else if (this.airPressure > 8.00 || this.airPressure < 4.00) {
